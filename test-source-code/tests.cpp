...
TEST_CASE("Test for right horizontal collisions between any two entities")
{
	auto collisions = std::make_shared<Collisions>();
	auto object1Xposition = 100.0f;
	auto object1Yposition = 350.0f;
	...
	//move first object towards second object such that the collide
	object1Xposition = 70.0f;
...
TEST_CASE("Test for top vertical collisions between any two entities")
{
	auto collisions = std::make_shared<Collisions>();
	...
	//move first object downwards such that it collides with second object
	object1Yposition = 340.0f;
...
TEST_CASE("Test if game ends when player collides with bomber")
{
	...
	auto bomber = std::make_shared<Bombers>(player);
	//set bomber to collide with player
	bomber->tests_SetPosition(playerXpos, playerYpos);
	...
	auto collision_manager = std::make_unique<CollisionsManager>();
	collision_manager->playerAndBomberCollisions(bombers,bomber_sprites, player);
...
TEST_CASE("Test if lander dies if it is shot by the player")
{
	std::vector<std::shared_ptr<Lander>> landers;
	std::vector<std::shared_ptr<LanderSprite>> lander_sprites;
	std::vector<std::shared_ptr<LaserSprite>> laser_sprites;
	...
	lander->tests_setPosition(landerXpos, landerYpos);
	landers.push_back(lander);
	...
	auto numberOfLanders = landers.size();

	CHECK(landersDestroyed == 0);
	CHECK(numberOfLanders == 1);

	manage_collision->landerAndLaserCollisions(landers, lander_sprites, laser_sprites, lasers, humanoids, player,
		manage_score, landersDestroyed);
...
//Score tests
TEST_CASE("Test if score is updted if player shoots lander")
{
	std::vector<std::shared_ptr<Lander>> landers;
	std::vector<std::shared_ptr<LanderSprite>> lander_sprites;
	std::vector<std::shared_ptr<LaserSprite>> laser_sprites;
...
TEST_CASE("Test if score is updated when player shoots bomber")
{
	std::vector<std::shared_ptr<Bombers>> bombers;
	std::vector<std::shared_ptr<BomberSprite>> bomber_sprites;
	std::vector<std::shared_ptr<LaserSprite>> laser_sprites;
	...
	auto playerScore = player->getScore();
	CHECK_EQ(playerScore, 0);
	manage_collision->LaserAndBomberCollisions(bombers, bomber_sprites, lasers, laser_sprites, player, manage_score);

	playerScore = player->getScore();
	CHECK_EQ(playerScore, 5);
}